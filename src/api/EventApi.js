export default class EventApi {
    ...
    isAttendingEvent(eventId, uid) {
        return this.database().ref(`/eventParticipants/${eventId}/${uid}/`).once('value')
            .then(snapshot => {
                let result = snapshot.val();
    ...
    create(event, user) {
        ...
        return this.database().ref(`/events/${newKey}`).set({id: newKey})
            .then(() => {
                let promises = [];
                promises.push(this.database().ref(`/eventAdmins/${newKey}/${user.uid}`).set(1));
                promises.push(this.database().ref(`/eventParticipants/${newKey}/${user.uid}`).set(true));
                promises.push(this.database().ref(`/userAcceptedEvents/${user.uid}/${newKey}`).set(true));
                promises.push(this.database().ref(`/eventData/${newKey}`).set({
                ...
                promises.push(this.database().ref(`/eventModules/${newKey}`).set({
                    details: {
                        enabled: true,
                        showTime: true,
                        showLocation: true,
